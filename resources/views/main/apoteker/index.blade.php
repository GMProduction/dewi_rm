    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex align-items-center">
                <div class="mr-auto p-2 f20 f-bold">List Apoteker</div>
                <a href="/admin/apoteker/add" class="btn my-button my-rounded pl-3 pr-3">
            ...
            @if(count($apoteker) > 0)
                <table id="my-table" class="table display">
                    ...
                    <tbody>
                    @foreach($apoteker as $v)
                        <tr>
                            <td>{{ $loop->index + 1 }}</td>
                            <td>{{ $v->nama }}</td>
                            <td>{{ $v->alamat }}</td>
                            <td class="text-center">
                                <a data-id="{{ $v->id }}" href="/admin/apoteker/edit/{{$v->id}}"><i
